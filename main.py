...
def resolve_performance_toggles(cli_args, report_option):
    ...
    def value_or_prompt(current_value, prompt_func):
        if current_value is not None:
            return current_value
        return prompt_func()

    if report_option in {"arc", "ads", "clickview", "paid_organic_terms"}:
        toggles["include_channel_types"] = value_or_prompt(
            cli_args.include_channel_type, helpers.include_channel_types
        )
    if report_option in {"arc", "ads", "clickview", "paid_organic_terms"}:
        toggles["include_campaign_info"] = value_or_prompt(
            cli_args.include_campaign_info, helpers.include_campaign_info
        )
    if report_option in {"ads", "clickview", "paid_organic_terms"}:
        toggles["include_adgroup_info"] = value_or_prompt(
            cli_args.include_adgroup_info, helpers.include_adgroup_info
        )
    if report_option in {"clickview", "paid_organic_terms"}:
        toggles["include_device_info"] = value_or_prompt(
            cli_args.include_device_type, helpers.include_device_info
        )
...
def build_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include channel type segmentation when available.",
    )
    parser.add_argument(
        "--no-include-channel-type",
        dest="include_channel_type",
        action="store_false",
        help="Exclude channel type segmentation when available.",
    )
    parser.set_defaults(include_channel_type=None)
    parser.add_argument(
        "--include-campaign-info",
        dest="include_campaign_info",
        action="store_true",
        help="Include campaign metadata when available.",
    )
    parser.add_argument(
        "--no-include-campaign-info",
        dest="include_campaign_info",
        action="store_false",
        help="Exclude campaign metadata when available.",
    )
    parser.set_defaults(include_campaign_info=None)
    parser.add_argument(
        "--include-adgroup-info",
        dest="include_adgroup_info",
        action="store_true",
        help="Include ad group metadata when available.",
    )
    parser.add_argument(
        "--no-include-adgroup-info",
        dest="include_adgroup_info",
        action="store_false",
        help="Exclude ad group metadata when available.",
    )
    parser.set_defaults(include_adgroup_info=None)
    parser.add_argument(
        "--include-device-type",
        dest="include_device_type",
        action="store_true",
        help="Include device segmentation when available.",
    )
    parser.add_argument(
        "--no-include-device-type",
        dest="include_device_type",
        action="store_false",
        help="Exclude device segmentation when available.",
    )
    parser.set_defaults(include_device_type=None)
    parser.add_argument(
        "--debug",
        action="store_true",
...
def resolve_report_scope(cli_args):
    ...
    while True:
        selection = prompts.report_menu()
        scope = consts.REPORT_SCOPE_MENU_LOOKUP.get(selection)
        if scope:
            cli_args.report_scope = scope
            return scope
        print("Invalid input, please select one of the indicated numbered options.")
